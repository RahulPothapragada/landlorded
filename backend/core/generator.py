...
def _build_report_html(analyses: list[LegalAnalysis], agreement_path: str) -> str:
    """Build HTML for the clause-by-clause audit report."""
...
def _generate_report(analyses: list[LegalAnalysis], agreement_path: str) -> Path:
    """Generate the clause-by-clause audit report PDF to disk."""
    html = _build_report_html(analyses, agreement_path)
...
def _build_notice_html(analyses: list[LegalAnalysis],
                       tenant_name: str, landlord_name: str, premises: str) -> str:
    """Build HTML for the legal notice."""
    fired = [a for a in analyses if a.fired]
...
def _generate_legal_notice(analyses: list[LegalAnalysis],
                           ...
                           premises: str) -> Path:
    """Generate the legal notice PDF to disk."""
    html = _build_notice_html(analyses, tenant_name, landlord_name, premises)
...
def _build_checklist_text(analyses: list[LegalAnalysis]) -> str:
    """Build evidence checklist as markdown string."""
    fired = [a for a in analyses if a.fired]
...
def _generate_checklist(analyses: list[LegalAnalysis]) -> Path:
    """Generate evidence checklist as markdown to disk."""
    text = _build_checklist_text(analyses)
...
# --- Bytes-returning variants for web backend ---

def generate_report_bytes(analyses: list[LegalAnalysis], agreement_path: str) -> bytes:
    """Generate report PDF and return as bytes."""
    html = _build_report_html(analyses, agreement_path)
    return weasyprint.HTML(string=html).write_pdf(stylesheets=[weasyprint.CSS(string=REPORT_CSS)])


def generate_notice_bytes(analyses: list[LegalAnalysis],
                          tenant_name: str, landlord_name: str, premises: str) -> bytes:
    """Generate legal notice PDF and return as bytes."""
    html = _build_notice_html(analyses, tenant_name, landlord_name, premises)
    return weasyprint.HTML(string=html).write_pdf(stylesheets=[weasyprint.CSS(string=NOTICE_CSS)])


def generate_checklist_text(analyses: list[LegalAnalysis]) -> str:
    """Generate evidence checklist and return as string."""
    return _build_checklist_text(analyses)
